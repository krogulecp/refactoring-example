...
class FilmService {
    ...
    private static String printFilms(List<Film> films) throws URISyntaxException, IOException {
        return films.stream().map(film -> film.getTitle() + " " + film.getLength()).collect(Collectors.joining("\n"));
    }

    public List<Film> findFilmsLongerThan(int filmLenght) throws URISyntaxException, IOException {
        ...
        for (var line : allFilmLines){
            ...
            if (!lineParts[2].isEmpty() && Integer.parseInt(lineParts[2]) > filmLenght) {
