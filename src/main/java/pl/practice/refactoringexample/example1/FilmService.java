...
class FilmService {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        System.out.println(printFilms());
    }

    private static String printFilms() throws URISyntaxException, IOException {
        return new FilmService().findFilmsLongerThan(120).stream().map(film -> film.getTitle() + " " + film.getLength()).collect(Collectors.joining("\n"));
    }

    public List<Film> findFilmsLongerThan(int lenght) throws URISyntaxException, IOException {
        ...
        for (var line : allFilmLines){
            ...
            if (!lineParts[2].isEmpty() && Integer.parseInt(lineParts[2]) > lenght) {
